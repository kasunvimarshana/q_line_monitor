...
GROUP_CONCAT(DISTINCT table_defect_type.name ORDER BY table_defect_type.id) AS defect_type_name,
GROUP_CONCAT(DISTINCT table_defect.name ORDER BY table_defect.defect_type) AS defect_name
...
if( (isset($queryResults)) && (is_array($queryResults)) ){
    ...
    $sheet ->getCell('E1')->setValue('Qty');
    $sheet ->getCell('F1')->setValue('State');
    $sheet ->getCell('G1')->setValue('Defect Type');
    $sheet ->getCell('H1')->setValue('Defect');
    ...
    foreach( $queryResults AS $key=>$value){
        ...
        $device_data_scan_var_qty = $value["device_data_scan_var_qty"];
        ...
        $sheet->setCellValueByColumnAndRow(4, $row, $device_data_scan_var_qty);
        $sheet->setCellValueByColumnAndRow(5, $row, $var_state_name);
        $sheet->setCellValueByColumnAndRow(6, $row, $defect_type_name);
