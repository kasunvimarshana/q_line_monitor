...
//try{
    ...
    */
    if(isset($REQUEST["imageDelete"])){
        $commonObjectClass->__set("imageDelete", $REQUEST["imageDelete"]);
    }
    if(isset($REQUEST["userName"])){
        $sysUser->setUserName($REQUEST["userName"]);
    }
    if(isset($REQUEST["userPassword"])){
        $sysUser->setUserPassword($REQUEST["userPassword"]);
    }
...
if(isset($REQUEST["submit"])){
    //try{
        ...
        if( (isset($FILES["image"])) && (!empty($FILES['image']["tmp_name"])) ){
           /* delete image */
           f_deleteContent( $sys_user->__get("image") );
           $image = $FILES["image"];
        ...
        }else if( $commonObjectClass->__get("imageDelete") != null ){
            /* delete image */
            f_deleteContent( $sys_user->__get("image") );
            $sys_user->__set("image");
        }
        /* execute */
        $sys_userBoImpl->f_update( $sys_user );
        /* commit */
        DB::commit();
        /* page redirect */
