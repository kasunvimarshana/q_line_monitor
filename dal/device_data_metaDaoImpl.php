...
class device_data_metaDaoImpl extends MyDaoImplClass implements DBDal{
    ...
    */
	public function f_insert($obj){
		...
		$sql = "INSERT INTO device_data_meta (device_data, meta_key, meta_value, sys_date, sys_time, dev_date, sys_meta) VALUES (:device_data, :meta_key, :meta_value, :sys_date, :sys_time, :dev_date, :sys_meta)";
    ...
    */
    public function f_update($obj){
		...
		$sql = "UPDATE device_data_meta SET device_data = :device_data, meta_key = :meta_key, meta_value,  = :meta_value, sys_date = :sys_date, sys_time = :sys_time, sys_meta = :sys_meta WHERE id = :id";
		$stmt = $this->dbCon->prepare($sql);
		$stmt->bindParam(':device_data', $device_data);
		$stmt->bindParam(':meta_key', $meta_key);
		$stmt->bindParam(':meta_value', $meta_value);
    ...
    */
    public function f_delete($id){
		$sql = "DELETE FROM device_data_meta WHERE id = :id LIMIT 1";
		$stmt = $this->dbCon->prepare($sql);
		$stmt->bindParam(':id', $id, PDO::PARAM_INT);
		$affected = $stmt->execute();
		return $affected;
    ...
    */
    public function f_search($id){
		$sql = "SELECT * FROM device_data_meta WHERE id = :id LIMIT 1";
		$stmt = $this->dbCon->prepare($sql);
		$stmt->bindParam(':id', $id, PDO::PARAM_INT);
		$stmt->execute();
        $stmt->setFetchMode(PDO::FETCH_CLASS|PDO::FETCH_PROPS_LATE, 'device_data_meta');
		$temp = $stmt->fetch();
        return $temp;
	}
    /*
    ...
    */
    public function f_list(){
        $temps = array();
		$sql = "SELECT * FROM device_data_meta";
		$stmt = $this->dbCon->prepare($sql);
		$stmt->execute();
        $stmt->setFetchMode(PDO::FETCH_CLASS|PDO::FETCH_PROPS_LATE, 'device_data_meta');
        $temps = $stmt->fetchAll();
    ...
    */
    public function f_searcWithParams($params){
        ...
        if(!empty($whereQ)){
            $sql = "SELECT * FROM device_data_meta WHERE ".implode(" AND ", $whereQ);
        }
		$stmt = $this->dbCon->prepare($sql);
