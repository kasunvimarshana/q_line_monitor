...
$offset = 0;
$draw = 0;
$defectBoImpl = new defectBoImpl();
$query1 = "SELECT COUNT(DISTINCT id) AS data_count FROM defect";
//$query1 = json_encode(array("query_type" => DBQ::CUSTOM_Q, "query" => $query1));
$query2 = "SELECT * FROM defect";
...
<?php
    ...
    try{
		...
		/* search value */
        if( (isset($REQUEST['defect_type'])) && (!empty($REQUEST['defect_type'])) ) {
			$temp = array();
			$defect_type = $REQUEST['defect_type'];
			if( is_array($defect_type) ){
				foreach($defect_type as $key=>$value){
					$temp["defect_type{$key}"] = array( 
						"key1"  => "defect_type", 
						"key2"  => "defect_type{$key}",
						"value" => $value, 
						"op"    => "=",
		...
		/* set where value */
		$queryParams = $defectBoImpl->manage_where_param_1($queryParams, $and_value, $or_value);
        /* limit value */
	...
	try{
        ...
        /* get data */
        $result = $defectBoImpl->f_query1($query2);
        if( isset($result) ){
        ...
        $result = $defectBoImpl->f_query1($query1);
        if( isset($result) ){
           $temp = $result->fetch();
