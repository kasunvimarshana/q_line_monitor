...
$query1 = "SELECT COUNT(DISTINCT id) AS data_count FROM var_line";
//$query1 = json_encode(array("query_type" => DBQ::CUSTOM_Q, "query" => $query1));
$query2 = "SELECT * FROM var_line";
...
<?php
    ...
    try{
		...
		/* search value */
        if( (isset($REQUEST['var_plant'])) && (!empty($REQUEST['var_plant'])) ) {
			$temp = array();
			$temp["var_plant"] = array( 
				"key1"  => "var_plant", 
				"key2"  => "var_plant",
				"value" => $REQUEST['var_plant'], 
				"op"    => "=",
		...
		/* set where value */
		$queryParams = $var_lineBoImpl->manage_where_param_1($queryParams, $and_value, $or_value);
        /* limit value */
	...
	try{
        ...
        /* get data */
        $result = $var_lineBoImpl->f_query1($query2);
        if( isset($result) ){
        ...
        $result = $var_lineBoImpl->f_query1($query1);
        if( isset($result) ){
           $temp = $result->fetch();
