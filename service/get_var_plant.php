...
$query1 = "SELECT COUNT(DISTINCT id) AS data_count FROM var_plant";
//$query1 = json_encode(array("query_type" => DBQ::CUSTOM_Q, "query" => $query1));
$query2 = "SELECT * FROM var_plant";
...
<?php
    ...
    try{
		...
		/* set where value */
		$queryParams = $var_plantBoImpl->manage_where_param_1($queryParams, $and_value, $or_value);
        /* limit value */
	...
	try{
        ...
        /* get data */
        $result = $var_plantBoImpl->f_query1($query2);
        if( isset($result) ){
        ...
        $result = $var_plantBoImpl->f_query1($query1);
        if( isset($result) ){
           $temp = $result->fetch();
